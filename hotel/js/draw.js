...
// Anchor hücre çizer — uzantı hücreler atlanır
function drawCell(c, f, cell){
  if(!cell.isAnchor) return;

  const {x, y} = cpos(c, f);
  const sz = CELL_SIZE[cell.type] || {w:1, h:1};
  const W  = sz.w * CW;
  const H  = sz.h * CH;

  // Hover: bu binanın herhangi bir hücresinin üstünde mi?
  const hov = hovCell &&
    hovCell.floor >= f && hovCell.floor < f + sz.h &&
    hovCell.col  >= c && hovCell.col  < c + sz.w;

  if(cell.type === T.EMPTY){
    if(hov){ctx.fillStyle='rgba(90,60,40,.08)'; ctx.fillRect(x,y,W,H);}
    return;
  }
  ...
  // Dekor (kırpılmış alan içinde)
  ctx.save(); ctx.beginPath(); ctx.rect(x+1,y+1,W-2,H-2); ctx.clip();
  drawDecor(x, y, cell.type, W, H);
  ctx.restore();

  // Dolu oda sıcak parlaması
  if(cell.occupied&&[T.STANDARD,T.DELUXE,T.SUITE].includes(cell.type)){
    ctx.fillStyle='rgba(255,170,55,.18)'; ctx.fillRect(x,y,W,H);
  }
  if(cell.dirty){
    ctx.fillStyle='rgba(178,138,88,.34)'; ctx.fillRect(x,y,W,H);
    ctx.font=`${MOBILE?8:11}px serif`; ctx.textAlign='right'; ctx.textBaseline='top';
    ctx.fillText('🧺', x+W-2, y+2);
  }
  if(cell.broken){
    ctx.fillStyle='rgba(231,108,90,.24)'; ctx.fillRect(x,y,W,H);
    ctx.font=`${MOBILE?10:13}px serif`; ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText('⚠️', x+W/2, y+H/2);
  }
  if(hov){ctx.fillStyle='rgba(90,60,40,.10)'; ctx.fillRect(x,y,W,H);}
  ctx.strokeStyle='#ecd8c8'; ctx.lineWidth=1; ctx.strokeRect(x+.5,y+.5,W-1,H-1);
}

// Dekor — W ve H parametresiyle çoklu hücreye uygun
function drawDecor(x, y, type, W, H){
  switch(type){
    case T.STANDARD: case T.DELUXE: case T.SUITE:{
      ...
      // Pencere sayısı: standart 2, deluxe 3, suite 4
      const nWin = type===T.SUITE?4:type===T.DELUXE?3:2;
      const ww   = Math.max(4, Math.round(W*0.12));
      const wh   = Math.round(H*0.42);
      const wy1  = Math.round(y+H*0.14);
      const gap  = (W - nWin*ww) / (nWin+1);
      for(let i=0; i<nWin; i++){
        const wx = Math.round(x + gap + i*(ww+gap));
        ctx.fillStyle=wfc; ctx.fillRect(wx,wy1,ww,wh);
        ctx.fillStyle=gc;  ctx.fillRect(wx+1,wy1+1,ww-2,wh-2);
      }
      ctx.fillStyle=type===T.SUITE?'#ecd0a0':type===T.DELUXE?'#c6d3e8':'#ccd8ea';
      ctx.fillRect(x,y+H-Math.round(H*.2),W,Math.round(H*.2));
      break;
    }
    case T.CORRIDOR:{
      ctx.fillStyle='#e7d0bb'; ctx.fillRect(x,y+H-Math.round(H*.25),W,Math.round(H*.25));
      ctx.strokeStyle='rgba(190,150,120,.6)'; ctx.lineWidth=1;
      ctx.beginPath(); ctx.moveTo(x,y+H-Math.round(H*.25)); ctx.lineTo(x+W,y+H-Math.round(H*.25)); ctx.stroke();
      ctx.fillStyle='rgba(255,215,130,.5)';
      ctx.beginPath(); ctx.arc(x+W/2,y+3,Math.max(2,CW*.08),0,Math.PI*2); ctx.fill();
      break;
    }
    case T.ELEVATOR:{
      const dw=Math.round((W-6)/2);
      const lg=ctx.createLinearGradient(x+3,0,x+3+dw,0);
      lg.addColorStop(0,'#cdbcea'); lg.addColorStop(1,'#bba6e2');
      ctx.fillStyle=lg; ctx.fillRect(x+3,y+2,dw,H-4);
      const rg=ctx.createLinearGradient(x+W/2+1,0,x+W/2+1+dw,0);
      rg.addColorStop(0,'#bba6e2'); rg.addColorStop(1,'#cdbcea');
      ctx.fillStyle=rg; ctx.fillRect(x+W/2+1,y+2,dw,H-4);
      ctx.fillStyle='#9a86c4'; ctx.fillRect(x+W/2-1,y+2,2,H-4);
      const arSz=Math.max(6,Math.round(H*.3));
      ctx.fillStyle='rgba(108,78,168,.85)'; ctx.font=`${arSz}px sans-serif`;
      ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('↕',x+W/2,y+H/2);
      break;
    }
    case T.STAIRS:{
      const steps=Math.max(3,Math.round(W/12));
      ctx.fillStyle='#9fcbb6';
      for(let i=0;i<steps;i++){
        const sw=(W-6)/steps;
        ctx.fillRect(x+3+i*sw,Math.round(y+H*.15+i*(H*.7/steps)),Math.ceil(sw),Math.max(2,Math.round(H*.12)));
      }
      break;
    }
    case T.RECEPTION:{
      drawReceptionCounter(x,y,W,H);
      // Monitörler — oda genişliğine göre
      const nMon = Math.max(1, Math.floor(W / CW));
      for(let i=0; i<nMon; i++){
        const mx = Math.round(x + (i+0.5)*(W/nMon));
        ctx.fillStyle='rgba(80,180,255,.3)';  ctx.fillRect(mx-6,Math.round(y+H*.18),12,Math.round(H*.22));
        ctx.fillStyle='rgba(80,200,255,.15)'; ctx.fillRect(mx-7,Math.round(y+H*.17),14,Math.round(H*.24));
      }
      break;
    }
    case T.RESTAURANT:{
      // Masa sayısı genişliğe göre
      const nTables = Math.max(2, Math.round(W / CW));
      const tw = Math.round(CW*.26);
      const tGap = (W - nTables*tw) / (nTables+1);
      for(let i=0;i<nTables;i++){
        const tx=Math.round(x+tGap+i*(tw+tGap));
        ctx.fillStyle='#dd8b7a'; ctx.fillRect(tx,y+4,tw,Math.round(H*.38));
        ctx.fillStyle='rgba(255,80,80,.15)'; ctx.fillRect(tx+1,y+5,tw-2,Math.round(H*.2));
      }
      drawRestaurantCounter(x,y,W,H);
      break;
    }
    case T.BAR:{
      drawBarCounter(x,y,W,H);
      const nb=Math.max(2,Math.round(W/14));
      for(let i=0;i<nb;i++){
        const bx=Math.round(x+4+i*(W-8)/nb);
        const bc=['rgba(0,200,80,.5)','rgba(200,120,0,.5)','rgba(100,0,200,.5)','rgba(0,120,200,.5)'][i%4];
        ctx.fillStyle=bc; ctx.fillRect(bx,Math.round(y+H*.1),Math.max(3,Math.round(CW*.08)),Math.round(H*.38));
      }
      break;
    }
    case T.POOL:{
      const pg=ctx.createLinearGradient(x,y+H*.35,x,y+H);
      pg.addColorStop(0,'#5cc0d2'); pg.addColorStop(1,'#3a9cb0');
      ctx.fillStyle=pg; ctx.fillRect(x+3,Math.round(y+H*.35),W-6,Math.round(H*.58));
      ctx.strokeStyle='rgba(255,255,255,.45)'; ctx.lineWidth=1;
      const step=Math.max(8,Math.round(CW*.18));
      for(let wx=x+5;wx<x+W-5;wx+=step){
        ctx.beginPath(); ctx.moveTo(wx,Math.round(y+H*.52));
        ctx.quadraticCurveTo(wx+step/2,Math.round(y+H*.44),wx+step,Math.round(y+H*.52)); ctx.stroke();
      }
      ctx.fillStyle='#a9d2da'; ctx.fillRect(x+3,Math.round(y+H*.34),W-6,Math.round(H*.04));
      break;
    }
  }
}

// Counter fonksiyonları (W ve H alır)
function drawReceptionCounter(x,y,W,H){
  ctx.fillStyle='#3fae6e'; ctx.fillRect(x+3,Math.round(y+H*.45),W-6,Math.round(H*.22));
  ctx.fillStyle='#6bd49a'; ctx.fillRect(x+3,Math.round(y+H*.45),W-6,Math.round(H*.06));
}
function drawBarCounter(x,y,W,H){
  ctx.fillStyle='#cb9c5c'; ctx.fillRect(x+3,y+H-Math.round(H*.3),W-6,Math.round(H*.3));
  ctx.fillStyle='#b9863f'; ctx.fillRect(x+3,y+H-Math.round(H*.3),W-6,Math.round(H*.06));
}
function drawRestaurantCounter(x,y,W,H){
  ctx.fillStyle='#dd8b7a'; ctx.fillRect(x+3,y+H-Math.round(H*.24),W-6,Math.round(H*.24));
  ctx.fillStyle='#ecb0a2'; ctx.fillRect(x+3,y+H-Math.round(H*.24),W-6,Math.round(H*.05));
}
...
function drawEntity(e,bodyColor,icon,voff=0,scale=1){
  ...
  const baseTop=py+hR*0.30, bodyTop=baseTop+bob;
...
function drawGhostColumn(gc){
  const isLeft=gc<0;
  const dist=isLeft?Math.abs(gc)-1:gc-COLS;
  const baseAlpha=Math.max(0.03,0.22-dist*0.07);
  const gfade=1-_nightF*0.82;
  const hov=hovCell&&hovCell.col===gc;
  const innermost=dist===0;
  ...
  if(innermost){
    const midF=Math.floor(FLOORS/2); const{x,y}=cpos(gc,midF);
...
function expandHotel(side){
  ...
  if(side==='right'){
    ...
    // Uzantı hücrelerin anchor referanslarını güncelle
    for(let f=0;f<FLOORS;f++) for(let c=0;c<COLS;c++){
      const cell=grid[f][c];
      if(!cell.isAnchor && cell.anchorC!==null) cell.anchorC++;
    }
...
function drawBuildPreview(){
  ...
  // Alanın uygunluğunu kontrol et
  let ok = money >= DEF[tool].cost;
  for(let dy=0;dy<sz.h&&ok;dy++) for(let dx=0;dx<sz.w&&ok;dx++){
    const c=col+dx, f=floor+dy;
    if(c>=COLS||f>=FLOORS||grid[f]?.[c]?.type!==T.EMPTY) ok=false;
  }
  // Footprint'i çiz
  const{x,y}=cpos(col,floor);
  const W=Math.min(sz.w*CW,(COLS-col)*CW);
  const H=sz.h*CH;
  ctx.fillStyle=ok?'rgba(46,204,113,.25)':'rgba(231,76,60,.25)'; ctx.fillRect(x,y,W,H);
  ctx.strokeStyle=ok?'#3fae6e':'#e74c3c'; ctx.lineWidth=2; ctx.strokeRect(x+1,y+1,W-2,H-2);
}
...
function drawGhostFloor(gf){
  const dist=gf-FLOORS;
  const baseAlpha=Math.max(0.03,0.22-dist*0.09);
  const gfade=1-_nightF*0.82;
  const innermost=dist===0;
...
function dayCycle(){
  ...
  const a=K[i], b=K[i+1], f=(p-a[0])/((b[0]-a[0])||1);
...
function render(){
  ...
  const dc=dayCycle(); _nightF=dc.night;
  ...
  // Bevel kenarlar (uzantı dahil tüm inşa hücreler)
  for(let f=0;f<FLOORS;f++) for(let c=0;c<COLS;c++){
  ...
  ctx.fillRect(0,gy,BX-WW,MB); ctx.fillRect(BX+COLS*CW+WW,gy,canvas.width,MB);
  ...
  ctx.fillStyle='#f06b48'; ctx.font=`bold ${enFontSz}px Fredoka, sans-serif`;
  ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('▲ GİRİŞ',entrX,gy+MB/2);
  ...
  if(dc.night>0.02){
    ...
    ctx.fillStyle=`rgba(34,26,62,${(dc.night*0.32).toFixed(3)})`; ctx.fillRect(BX-WW,gy,COLS*CW+WW*2,MB);
    for(let f=0;f<FLOORS;f++) for(let c=0;c<COLS;c++){
      const cell=grid[f][c];
      const lit=(cell.occupied&&[T.STANDARD,T.DELUXE,T.SUITE].includes(cell.type))
        ||cell.type===T.RECEPTION||cell.type===T.RESTAURANT||cell.type===T.BAR;
      if(!lit||!cell.isAnchor) continue;
      const sz=CELL_SIZE[cell.type]||{w:1,h:1};
      const{x,y}=cpos(c,f); const W=sz.w*CW;
      const gl=ctx.createRadialGradient(x+W/2,y+CH/2,1,x+W/2,y+CH/2,W*0.72);
      gl.addColorStop(0,`rgba(255,206,120,${(dc.night*0.78).toFixed(3)})`);
      gl.addColorStop(1,'rgba(255,206,120,0)');
      ctx.fillStyle=gl; ctx.fillRect(x-2,y-2,W+4,CH+4);
    }
  }
  ...
  for(const s of _stationed){
    const anchorCell = grid[s.floor]?.[s.col];
    const cs = COUNTER_STATION[anchorCell?.type];
    const icon = s.type==='receptionist'?'💁':'🤵';
    drawEntity(s, STAFF_DEF[s.type].color, icon, cs?cs.voff*CH:0, 0.9);
    if(cs){
      const{x,y}=cpos(s.col,s.floor);
      const sz=CELL_SIZE[anchorCell?.type]||{w:1,h:1};
      cs.front(x,y,sz.w*CW,CH);
    }
  }
