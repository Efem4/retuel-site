...
function fireStaff(type){
  // Önce boştaki, yoksa aktif olanı çıkar
  let idx = -1;
  for(let i = staffArr.length-1; i >= 0; i--){
    if(staffArr[i].type === type && staffArr[i].state === 'idle'){ idx = i; break; }
  }
  if(idx === -1){
    for(let i = staffArr.length-1; i >= 0; i--){
      if(staffArr[i].type === type){ idx = i; break; }
    }
  }
  if(idx === -1){ toast(`${STAFF_DEF[type].name} yok!`); return; }
  const s = staffArr[idx];
  // Üstlendiği görevi serbest bırak
  if(s.tC >= 0 && s.tF >= 0){
    const cell = grid[s.tF]?.[s.tC];
    if(cell && cell.sId === s.id) cell.sId = null;
  }
  staffArr.splice(idx, 1);
  updateStaffUI();
  toast(`${STAFF_DEF[type].name} işten çıkarıldı.`);
}
...
function assignMaid(s){
  if(s.tasksToday >= 6) return;  // günlük kapasite: 6 oda
