...
function buildGrid(){
  ...
  // Elevator tüm katlarda col 0
  for(let f=0; f<FLOORS; f++) grid[f][0] = mkCell(T.ELEVATOR);
  // Zemin kat: koridor şeridi
  for(let c=1; c<COLS; c++) grid[0][c] = mkCell(T.CORRIDOR);
  // Resepsiyon (2×1) — zemin kordior üzerine doğrudan yaz
  grid[0][2] = mkCell(T.RECEPTION, true, null, null);
  grid[0][3] = mkCell(T.RECEPTION, false, 2, 0);
  // 1. kat: koridor şeridi + standart odalar (her oda koridora bitişik)
  for(let c=1; c<=6; c++) grid[1][c] = mkCell(T.CORRIDOR);
  grid[1][3] = mkCell(T.STANDARD);  // col 2 koridoruna bitişik ✓
  grid[1][5] = mkCell(T.STANDARD);  // col 4 koridoruna bitişik ✓
}
...
// ── Koridor erişim kontrolü ──
function hasCorridorAccess(col, floor){
  const INFRA = [T.CORRIDOR, T.ELEVATOR, T.STAIRS];
  for(const dc of [-1, 1]){
    const nc = col + dc;
    if(nc < 0 || nc >= COLS) continue;
    if(INFRA.includes(grid[floor]?.[nc]?.type)) return true;
  }
  return false;
}
